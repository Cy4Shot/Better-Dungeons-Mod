...
public class PlayerKeyCreationTablePlacingData extends WorldSavedData {

	protected static final String DATA_NAME = BetterDungeons.MOD_ID + "_PlayerKeyCreationTablePlacing";

	private Map<UUID, PlayerPlacingStats> playerMap = new HashMap<>();

	public PlayerKeyCreationTablePlacingData() {
		super(DATA_NAME);
	}

	public PlayerKeyCreationTablePlacingData(String name) {
		super(name);
	}
	...
	public static PlayerKeyCreationTablePlacingData get(ServerWorld world) {
		return world.getServer().func_241755_D_().getSavedData().getOrCreate(PlayerKeyCreationTablePlacingData::new, DATA_NAME);
	}

	public PlayerKeyCreationTablePlacingData setCanPlace(PlayerEntity player, boolean c) {
