...
public class BlockStateSerializationHelper {
	...
	public static CompoundNBT serializeBlockStateToNBT(BlockState state) {
		...
		if (state.getProperties().size() > 0) {
			...
			final Property property : state.getProperties()) {
	...
	public static BlockState deserializeBlockState(CompoundNBT nbt) {
		if (!nbt.contains("block")) {
			BetterDungeons.LOGGER.warn("NBT compound {} is not a blockstate", nbt);
			return null;
		}
		...
		if (block == null) {
			BetterDungeons.LOGGER.warn("Unknown block {} in NBT package", nbt.getString("block"));
			return null;
		}
		...
		if (nbt.contains("properties")) {
			...
			for (String propertyName : propertiesTag.keySet()) {
				...
				if (blockProperty == null) {
					BetterDungeons.LOGGER.warn("The property '{}' is not valid for block {}", propertyName, blockId);
					continue;
				}
				...
				if (!propValue.isPresent()) {
					BetterDungeons.LOGGER.warn("The property '{}' with value '{}' could not be parsed!", propertyName, valueString);
					continue;
				}
				...
				try {
					...
					BetterDungeons.LOGGER.warn("Failed to update state for block {}. The mod that adds this block has issues.",
				...
				if (blockProperty != null) {
					...
					if (propValue.isPresent()) {
						...
						try {
							...
							BetterDungeons.LOGGER.error("Failed to read state for block {}. The mod that adds this block has issues.",
									block.getRegistryName());
						}
					}
				}
	...
	public static BlockState deserializeBlockState(JsonObject json) {
		...
		// If the properties member exists, attempt to assign properties to the block
		// state.
			...
			if (propertiesElement.isJsonObject()) {
				...
				for (final Map.Entry<String, JsonElement> property : props.entrySet()) {
					...
					if (blockProperty != null) {
						...
						if (property.getValue().isJsonPrimitive()) {
							...
							if (propValue.isPresent()) {
								...
								try {
									...
									BetterDungeons.LOGGER.error(
											"Failed to update state for block {}. The mod that adds this block has issues.",
											block.getRegistryName());
								}
							}
								...
								throw new JsonSyntaxException(
										"The property " + property.getKey() + " with value " + valueString + " could not be parsed!");
							}
						}

						else {

							throw new JsonSyntaxException("Expected property value for " + property.getKey()
									+ " to be primitive string. Got " + JSONUtils.toString(property.getValue()));
						}
					}
						...
						throw new JsonSyntaxException(
								"The property " + property.getKey() + " is not valid for block " + block.getRegistryName());
					}
				}
			}
