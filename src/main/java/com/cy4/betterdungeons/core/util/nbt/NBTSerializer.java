...
public class NBTSerializer {
	...
	public static final <T extends INBTSerializable> CompoundNBT serialize(T object)
			throws IllegalAccessException, UnserializableClassException {
	...
	@SuppressWarnings({ "unchecked", "rawtypes" })
	private static final <T, U extends T> INBT objectToTag(Class<T> clazz, U obj)
			throws IllegalAccessException, UnserializableClassException {
		...
		if (clazz.isAssignableFrom(Byte.class))
			...
			return ByteNBT.valueOf(((Boolean) obj) ? (byte) 1 : (byte) 0);
		...
		if (INBTSerializable.class.isAssignableFrom(clazz))
			...
			return serializeEntry((Map.Entry) obj);
	...
	@SuppressWarnings("unchecked")
	private static final <K, V> CompoundNBT serializeEntry(Map.Entry<K, V> entry)
			throws UnserializableClassException, IllegalAccessException {
	...
	private static final <K, V> CompoundNBT serializeEntry(Map.Entry<? extends K, ? extends V> entry, Class<K> keyClass,
			Class<V> valueClass) throws IllegalAccessException, UnserializableClassException {
	...
	public static final <T extends INBTSerializable> T deserialize(Class<T> definition, CompoundNBT data)
			throws IllegalAccessException, InstantiationException, UnserializableClassException {
	...
	public static final <T extends INBTSerializable> void deserialize(T instance, CompoundNBT data,
			boolean interpretMissingFieldValuesAsNull) throws IllegalAccessException, InstantiationException, UnserializableClassException {
	...
	private static <T> Collection<T> deserializeCollection(ListNBT list, Class<? extends Collection> colClass, Class<T> subclass,
			Type subtype) throws InstantiationException, IllegalAccessException, UnserializableClassException {
	...
	private static <K, V> Map<K, V> deserializeMap(ListNBT map, Class<? extends Map> mapClass, Class<K> keyClass, Type keyType,
			Class<V> valueClass, Type valueType) throws InstantiationException, IllegalAccessException, UnserializableClassException {
	...
	@SuppressWarnings({ "unchecked", "rawtypes" })
	private static <T> T tagToObject(INBT tag, Class<T> clazz, Type subtype)
			throws IllegalAccessException, InstantiationException, UnserializableClassException {

		if (clazz.isAssignableFrom(Object.class) || clazz.isAssignableFrom(Number.class) || clazz.isAssignableFrom(CharSequence.class)
				|| clazz.isAssignableFrom(Serializable.class) || clazz.isAssignableFrom(Comparable.class))

			throw new UnserializableClassException(clazz);

		if (clazz.isAssignableFrom(Byte.class))
			return (T) Byte.valueOf(((ByteNBT) tag).getByte());
		else if (clazz.isAssignableFrom(Boolean.class))
			return (T) Boolean.valueOf(((ByteNBT) tag).getByte() != 0);
		else if (clazz.isAssignableFrom(Short.class))
			return (T) Short.valueOf(((ShortNBT) tag).getShort());
		else if (clazz.isAssignableFrom(Integer.class))
			return (T) Integer.valueOf(((IntNBT) tag).getInt());
		else if (clazz.isAssignableFrom(Long.class))
			return (T) Long.valueOf(((LongNBT) tag).getLong());
		else if (clazz.isAssignableFrom(Float.class))
			return (T) Float.valueOf(((FloatNBT) tag).getFloat());
		else if (clazz.isAssignableFrom(Double.class))
			return (T) Double.valueOf(((DoubleNBT) tag).getDouble());
		else if (clazz.isAssignableFrom(byte[].class))
			return (T) ((ByteArrayNBT) tag).getByteArray();
		else if (clazz.isAssignableFrom(Byte[].class))
			return (T) ArrayUtils.toObject(((ByteArrayNBT) tag).getByteArray());
		else if (clazz.isAssignableFrom(String.class))
			return (T) ((StringNBT) tag).getString();
		else if (clazz.isAssignableFrom(int[].class))
			return (T) ((IntArrayNBT) tag).getIntArray();
		else if (clazz.isAssignableFrom(Integer[].class))
	...
	private static int getIDFromClass(Class<?> clazz) {
		...
		if (clazz.isAssignableFrom(byte.class) || clazz.isAssignableFrom(Byte.class) || clazz.isAssignableFrom(boolean.class)
