...
public class UIHelper {
	...
	public static void renderContainerBorder(AbstractGui gui, MatrixStack matrixStack, Rectangle screenBounds, int u, int v, int lw, int rw,
			int th, int bh, int contentColor) {
	...
	public static void renderContainerBorder(AbstractGui gui, MatrixStack matrixStack, int x, int y, int width, int height, int u, int v,
			int lw, int rw, int th, int bh, int contentColor) {
	...
	public static void renderLabelAtRight(AbstractGui gui, MatrixStack matrixStack, String text, int x, int y) {
		...
		fontRenderer.drawString(matrixStack, text, 14 + gap, 9, 0xFF_443a1b);
	...
	public static int renderWrappedText(MatrixStack matrixStack, IFormattableTextComponent text, int maxWidth, int padding) {
		...
		List<ITextProperties> lines = getLines(TextComponentUtils.func_240648_a_(text.deepCopy(), text.getStyle()), maxWidth - 3 * padding);
		List<IReorderingProcessor> processors = LanguageMap.getInstance().func_244260_a(lines);

		for (int i = 0; i < processors.size(); i++) {
			fontRenderer.func_238422_b_(matrixStack, processors.get(i), padding, (10 * i) + padding, 0xFF_192022);
		}
