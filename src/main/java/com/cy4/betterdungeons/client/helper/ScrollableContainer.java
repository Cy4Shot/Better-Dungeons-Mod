...
public class ScrollableContainer extends AbstractGui {
	...
	public void mouseMoved(double mouseX, double mouseY) {
		if (scrolling) {
			...
			yOffset = MathHelper.clamp(scrollingOffsetY + (int) (deltaOffset * innerHeight / scrollBounds.getHeight()), 0,
	...
	public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
		...
		textureManager.bindTexture(UI_RESOURCE);
		UIHelper.renderContainerBorder(this, matrixStack, renderableBounds, 14, 44, 2, 2, 2, 2, 0xFF_8B8B8B);

		UIHelper.renderOverflowHidden(matrixStack, (ms) -> fill(ms, renderableBounds.x0 + 1, renderableBounds.y0 + 1,
				renderableBounds.x1 - 1, renderableBounds.y1 - 1, 0xFF_8B8B8B), (ms) -> {
					ms.push();
