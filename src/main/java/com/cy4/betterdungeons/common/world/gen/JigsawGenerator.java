...
public class JigsawGenerator {
	public static void func_242837_a(DynamicRegistries p_242837_0_, VillageConfig p_242837_1_, JigsawManager.IPieceFactory p_242837_2_,
			ChunkGenerator p_242837_3_, TemplateManager p_242837_4_, BlockPos p_242837_5_, List<? super AbstractVillagePiece> p_242837_6_,
			Random p_242837_7_, boolean p_242837_8_, boolean p_242837_9_) {
		Structure.init();
		...
		if (p_242837_1_.func_236534_a_() > 0) {
			...
			AxisAlignedBB axisalignedbb = new AxisAlignedBB(i - maxRange, k - maxRange, j - maxRange, i + maxRange + 1, k + maxRange + 1,
					j + maxRange + 1);
			Assembler jigsawmanager$assembler = new Assembler(mutableregistry, p_242837_1_.func_236534_a_(), p_242837_2_, p_242837_3_,
					p_242837_4_, p_242837_6_, p_242837_7_);
			jigsawmanager$assembler.availablePieces.addLast(new Entry(abstractvillagepiece,
					new MutableObject<>(VoxelShapes.combineAndSimplify(VoxelShapes.create(axisalignedbb),
							VoxelShapes.create(AxisAlignedBB.toImmutable(mutableboundingbox)), IBooleanFunction.ONLY_FIRST)),
					k + maxRange, 0));
	...
	static final class Assembler {
		...
		private Assembler(Registry<JigsawPattern> p_i242005_1_, int p_i242005_2_, JigsawManager.IPieceFactory p_i242005_3_,
				ChunkGenerator p_i242005_4_, TemplateManager p_i242005_5_, List<? super AbstractVillagePiece> p_i242005_6_,
				Random p_i242005_7_) {
		...
		private void func_236831_a_(AbstractVillagePiece p_236831_1_, MutableObject<VoxelShape> p_236831_2_, int p_236831_3_,
				int currentDepth, boolean p_236831_5_) {
			...
			label139: for (Template.BlockInfo template$blockinfo : jigsawpiece.getJigsawBlocks(this.templateManager, blockpos, rotation,
					this.rand)) {
					...
					if (fallbackJigsawPattern.isPresent() && (fallbackJigsawPattern.get().getNumberOfPieces() != 0
						...
						for (JigsawPiece jigsawpiece1 : list) {
							...
							for (Rotation rotation1 : Rotation.shuffledRotations(this.rand)) {
								List<Template.BlockInfo> list1 = jigsawpiece1.getJigsawBlocks(this.templateManager, BlockPos.ZERO,
										rotation1, this.rand);
								MutableBoundingBox mutableboundingbox1 = jigsawpiece1.getBoundingBox(this.templateManager, BlockPos.ZERO,
										rotation1);
								int i1;
								if (p_236831_5_ && mutableboundingbox1.getYSize() <= 16) {
									i1 = list1.stream().mapToInt((p_242841_2_) -> {
										if (!mutableboundingbox1.isVecInside(
												p_242841_2_.pos.offset(JigsawBlock.getConnectingDirection(p_242841_2_.state)))) {
											return 0;
										} else {
								...
								for (Template.BlockInfo template$blockinfo1 : list1) {
									if (JigsawBlock.hasJigsawMatch(template$blockinfo, template$blockinfo1)) {
										...
										BlockPos blockpos4 = new BlockPos(blockpos2.getX() - blockpos3.getX(),
												blockpos2.getY() - blockpos3.getY(), blockpos2.getZ() - blockpos3.getZ());
										MutableBoundingBox mutableboundingbox2 = jigsawpiece1.getBoundingBox(this.templateManager,
												blockpos4, rotation1);
										...
										if (flag && flag2) {
											...
											if (k == -1) {
												k = this.chunkGenerator.getNoiseHeight(blockpos1.getX(), blockpos1.getZ(),
										...
										if (!VoxelShapes.compare(mutableobject1.getValue(),
												VoxelShapes.create(AxisAlignedBB.toImmutable(mutableboundingbox3).shrink(0.25D)),
											...
											AbstractVillagePiece abstractvillagepiece = this.pieceFactory.create(this.templateManager,
													jigsawpiece1, blockpos5, l2, rotation1, mutableboundingbox3);
											int i3;
											if (flag) {
												i3 = i + j;
											} else if (flag2) {
											...
											p_236831_1_.addJunction(new JigsawJunction(blockpos2.getX(), i3 - j + j3, blockpos2.getZ(), l1,
													jigsawpattern$placementbehaviour1));
											...
											if (abstractvillagepiece.getBoundingBox().minY > 0
												...
												if (currentDepth + 1 <= this.maxDepth) {
													this.availablePieces
															.addLast(new Entry(abstractvillagepiece, mutableobject1, l, currentDepth + 1));
												}
											}
						...
						BetterDungeons.LOGGER.warn("Empty or none existent fallback pool: {}", (Object) resourcelocation1);
					}
				} else {
