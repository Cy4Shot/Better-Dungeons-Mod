...
public class AOEGoal<T extends MobEntity> extends GoalTask<T> {
	...
	public void tick() {
		...
		if (!this.started && this.delay < 0 && this.getEntity().isOnGround()) {
			...
			((ServerWorld) this.getWorld()).spawnParticle(ParticleTypes.EXPLOSION, this.getEntity().getPosX() + 0.5D,
					this.getEntity().getPosY() + 0.1D, this.getEntity().getPosZ() + 0.5D, 10, this.getRandom().nextGaussian() * 0.02D,
					this.getRandom().nextGaussian() * 0.02D, this.getRandom().nextGaussian() * 0.02D, 1.0D);
		...
		if (this.started) {
			...
			this.getWorld().getEntitiesWithinAABB(LivingEntity.class, new AxisAlignedBB(this.shockwave).grow(max, max, max), e -> {
				...
				Vector3d direction = new Vector3d(e.getPosX() - this.shockwave.getX(), e.getPosY() - this.shockwave.getY(),
