...
public class SlimeSpikesEntity extends Entity {
	...
	public SlimeSpikesEntity(EntityType<? extends SlimeSpikesEntity> p_i50170_1_, World p_i50170_2_) {
		super(p_i50170_1_, p_i50170_2_);
	}

	public SlimeSpikesEntity(World worldIn, double x, double y, double z, float p_i47276_8_, int p_i47276_9_, LivingEntity casterIn) {
		this(EntityTypesInit.SLIME_SPIKES.get(), worldIn);
		...
		if (this.world.isRemote) {
			...
			if (this.warmupDelayTicks == -8) {
				for (LivingEntity livingentity : this.world.getEntitiesWithinAABB(LivingEntity.class,
						this.getBoundingBox().grow(0.2D, 0.0D, 0.2D))) {
					if (!(livingentity instanceof EnderSlimeEntity))
	...
	public void handleStatusUpdate(byte id) {
		...
		if (id == 4) {
			...
			if (!this.isSilent()) {
				this.world.playSound(this.getPosX(), this.getPosY(), this.getPosZ(), SoundEvents.ENTITY_EVOKER_FANGS_ATTACK,
						this.getSoundCategory(), 1.0F, this.rand.nextFloat() * 0.2F + 0.85F, false);
			}
		}
